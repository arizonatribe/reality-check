...
const { resolvePathIfExists, resolveGlobIfExists } = require("@vanillas/cli-toolkit")
...
function listAllFilesInFolder(dir) {
  ...
  for (let i = 0, len = matches.length; i < len; i++) {
    ...
    } else if (resolvePathIfExists(resolvedPath)) {
...
/**
 * Resolve a list of glob patterns to absolute file paths (if they exist)
 *
 * @function
 * @name resolveGlobbedFiles
 * @param {Array<string>} globs A list of one or more glob patterns to resolve from
 * @param {string} [cwd] A base directory from which to resolve
 * @returns {Array<string>} All the absolute file paths matching the glob patterns
 */
function resolveGlobbedFiles(globs, cwd) {
  return [].concat(
    ...globs.map(glob => resolveGlobIfExists(glob, cwd))
 ...
 * @param {string} baseDir A base directory from which to (recursively) search
 * @param {Array<string>} [filterPatterns] Optional regex filters to apply to the resolved file paths
 * @returns {Array<string>} A list of all (absolute) file paths which represent benchmark tests
 */
function findBenchmarkTests(baseDir, filterPatterns = []) {
  let files = listAllFilesInFolder(baseDir)

  if (toStringArray(filterPatterns).length) {
    const patterns = toStringArray(filterPatterns).map(strPattern => new RegExp(strPattern))
    files = files.filter(file => patterns.some(patt => patt.test(file)))
  }
